...
- (void)createNameButtons
{
    ...
    //顶部tabbar的总长度
    CGFloat topScrollViewContentWidth;
    //每个tab偏移量
    CGFloat xOffset;
    xOffset = kWidthOfButtonMargin;
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        float buttonWidth;
        if (_viewArray.count < 5 && _viewArray.count > 0 ){
            buttonWidth = 320 / _viewArray.count;
        }else{
        ...
        //计算下一个tab的x偏移量
        xOffset += buttonWidth + kWidthOfButtonMargin;
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    [self.slideSwitchViewDelegate slideScrollViewWillBeginDragging:scrollView];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [self.slideSwitchViewDelegate slideScrollViewDidScroll:scrollView];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    [self.slideSwitchViewDelegate slideScrollViewDidEndDecelerating:scrollView];
}
...
{
    [self selectNameButton:[_buttonArray objectAtIndex:page]];
}
